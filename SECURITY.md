...
- **Checked release tags.** Release tags are annotated and created by `scripts/tag-release.sh`; only maintainers can create `v*` tags (tag ruleset). The release workflow refuses a lightweight tag, one not on `main`, or one that doesn't match the versions committed at it, and publishes to PyPI and npm only if both packages built.
